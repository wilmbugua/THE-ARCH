# Simple HttpClient-based proxy and static server
    ...
    try {
        ...
        if ($req.Url.AbsolutePath.StartsWith('/api/') -or $req.Url.AbsolutePath.StartsWith('/ws/')) {
            ...
            try {
                ...
                # Copy headers (avoid restricted ones)
                ...
                foreach ($hdr in $resp.Headers) {
                    try { $ctx.Response.Headers.Add($hdr.Key, ($hdr.Value -join ',')) } catch { }
                }
                if ($resp.Content) {
                    foreach ($hdr in $resp.Content.Headers) {
                        try { $ctx.Response.Headers.Add($hdr.Key, ($hdr.Value -join ',')) } catch { }
                    }
